...
PLP->>Card : 注入 onQuickAdd/onEdit/onDeleteAsk/...
Card-->>PLP : 渲染完成
PLP->>Card : 用户点击卡片
...
## 结论
PlantCard 通过清晰的参数与事件设计，实现了植物信息展示、任务进度呈现与快捷操作入口的统一。其数据加载与光照动画增强了用户体验，同时与页面层解耦，便于在植物列表等场景中复用与扩展。建议在集成时关注事件回调的完整性与数据库查询的健壮性，以确保组件稳定运行。
...
- 在 PlantListPage 中渲染 PlantCard：
  ...
  - 注入 onQuickAdd/onEdit/onDeleteAsk/onOpenLogs/onOpenMetrics 等事件
